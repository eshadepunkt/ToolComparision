...
export function ReadCSVLine(
    csvLine: string, seperator: string = ',', rangeEscape: string = '"', outCommenter: string = '#'
    ): Array<string> {
        let indexes: Array<number> = [];
        let csv: string = csvLine;

        if (csvLine === "" || csvLine[0] == outCommenter)
            return [];
...
function GetEndIndexOfRangeEscape(csvLineReference: {csvLine: string}, currentIndex: number, rangeEscape: string = '"'): number {
    //Remove Escape Sequence
    csvLineReference.csvLine = 
        StringRemove(csvLineReference.csvLine, currentIndex, 1);
...
function EscapeRangeEscape(csvLineReference: {csvLine: string}, currentIndex: number, rangeEscape: string = '"'): boolean {
    if (currentIndex < csvLineReference.csvLine.length)
    {
...
function SplitCSV(csvLine: string, commaPositions: Array<number>): Array<string> {
    let splitCSV: Array<string> = [];
...
function GetCommaPositions(indexes: Array<number>, lineLenght: number): Array<number> {
...
function WriteCSVLine(
    ...
    seperator: string = ',', rangeEscape: string = '"', 
    autoEscape: boolean = true, escapeAll: boolean = true
    ): string {
    let csvLines: string = "";
    ...
    for (let i = 0; i < linesCount; i++)
    {
        ...
        for (let k = 0; k < entryCount; k++)
        {
            ...
            if (autoEscape)
            {
                let entr: string = entry.replace(rangeEscape, (rangeEscape + rangeEscape));
                //If the entry is not empty or it does not only contain rangeEscapes OR if the entry contains a separator
                if (escapeAll && (StringTrim(entr, rangeEscape) !== "") || entr.includes(seperator))
                {
...
export function WriteCSVComment(comment: string, outCommenter: string = '#'): string
{
...
export function StringRemove(str: string, startIndex: number, count: number): string {
